...
function newCards(order) {
	...
	var cardClient = $("<div>").addClass("card").attr({"id": "card-client"})
		...
		.append($("<div>").addClass("card-details")
			...
			//.append($("<div>").addClass("card-text-small").html("&nbsp"))
	...
	//var cardAddress = $("<div>").addClass(order.status.name.replace(/ /g, "-"));
	//var cardDeadline = $("<div>").addClass(order.status.name.replace(/ /g, "-"));

	var cards = $("<div>").attr({"id": "cards"})
		.append(cardStatus)
		.append(cardClient)
		.append(cardStatus.clone())
		.append(cardStatus.clone());
